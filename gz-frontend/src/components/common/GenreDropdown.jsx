// src/components/common/GenreDropdown.jsx — With Mobile Support
import { useState, useEffect, useRef } from "react";
import { Link, useNavigate } from "react-router-dom";
import moviesApi from "../../api/moviesApi";

export default function GenreDropdown({ mobile = false }) { // ← NEW prop
  ...
  const handleGenreClick = (genre) => {
    setOpen(false);
    navigate(`/movies?genre=${genre.slug}`); // ← use slug
  };
    ...
    return (
      <div className="space-y-1">
        ...
        {loading ? (
          ...
          <div className="px-4 py-3 text-gray-500 text-sm">មិនទាន់មានប្រភេទ</div>
        ) : (
          genres.map((genre) => (
            <button
              key={genre.id}
