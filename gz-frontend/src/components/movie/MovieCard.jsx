// src/components/movie/MovieCard.jsx — Netflix Style Card (Fixed & Clean)
...
export default function MovieCard({ movie, listView = false }) {
  ...
  if (!movie) {
    return null; // ឬ return fallback UI
  }
  ...
  // Only show badge for VIP or Paid
        ...
        <div className="aspect-[2/3] relative overflow-hidden rounded-xl bg-gray-900">
          ...
          {(imageError || !posterUrl) && (
            <div className="absolute inset-0 bg-gradient-to-br from-gray-800 to-gray-900 flex flex-col items-center justify-center gap-3 z-10">
              ...
              <span className="text-xs text-gray-500 text-center px-3 line-clamp-2">
                {title}
              </span>
            </div>
          )}
          ...
          {!imageError && posterUrl && (
            <img
              ...
              className={`
                ...
                ${isHovered ? 'scale-110 blur-[2px] brightness-50' : 'scale-100 blur-0 brightness-100'}
              `}
              onLoad={() => setImageLoaded(true)}
              onError={() => setImageError(true)}
              loading="lazy"
