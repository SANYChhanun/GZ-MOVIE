...
export default function MembershipPlanManagementPage() {
  ...
  const [editingPlan, setEditingPlan] = useState(null); // ← plan object being edited, or null
  ...
  return (
    <>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
        ...
        {/* Special / PPV — not a MembershipPlan row, sourced from purchases app */}
        <div className="rounded-xl p-5 border border-dashed flex flex-col gap-4 bg-slate-900 border-slate-800">
          ...
          <ul className="text-sm text-slate-400 flex flex-col gap-1.5 flex-1">
            <li>· No subscription — unlock one movie at a time</li>
            <li>· 1-month access per purchase</li>
            <li>· Priced per title (see Movies page)</li>
          </ul>
          <div className="text-xs text-slate-500 pt-3 border-t border-slate-800">
            {ppvStats ? `${ppvStats.active_purchasers} active purchasers · ${ppvStats.active_purchase_count} unlocks` : "—"}
          </div>
          <button
            disabled
