// src/components/movie/MovieCard.jsx â€” Netflix Style Card (Fixed)
...
export default function MovieCard({ movie, listView = false }) {
  ...
  const posterUrl = getPosterUrl();  // â† áž”áŸ’ážšáž¾ function áž“áŸáŸ‡
  const title = movie?.title || movie?.name || 'áž‚áŸ’áž˜áž¶áž“áž…áŸ†ážŽáž„áž‡áž¾áž„';
  ...
  const accessLabel = isMember ? 'VIP' : 'áž‘áž·áž‰';
        ...
        `}>
          ...
          <div className="flex-1 py-3 pr-4">
            ...
            <div className="flex items-center gap-2 mt-1 text-xs text-gray-400">
              ...
              {year && <span>{year}</span>}
              {duration && <span>{duration} áž“áž¶áž‘áž¸</span>}
            </div>
            {genres.length > 0 && (
          ...
          {isNew && (
            <div className="absolute top-2.5 left-2.5 z-30">
              <span className="bg-red-600 text-white text-[10px] px-2.5 py-1 rounded-full font-bold uppercase tracking-wider shadow-xl shadow-red-600/40 flex items-center gap-1">
                <i className="bi bi-lightning-fill text-[8px]"></i>ážáŸ’áž˜áž¸
              </span>
            </div>
          )}
          ...
          <div className="absolute bottom-0 left-0 right-0 z-30 p-3">
            ...
            `}>
              ...
              <div className="flex items-center gap-1.5 text-red-400 text-xs font-bold hover:text-red-300 transition-colors group/link">
                ...
                <span>áž˜áž¾áž›áž¥áž¡áž¼ážœáž“áŸáŸ‡</span>
